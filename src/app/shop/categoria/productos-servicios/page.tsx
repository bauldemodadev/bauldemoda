...
const segmentProducts = (products: Product[]) => {
  ...
  products.forEach(product => {
    ...
    } else if (category.includes('gift cards')) {
      segments.giftCards.push(product);
    } else if (category.includes('ebooks')) {
      segments.ebooks.push(product);
    } else if (category.includes('insumos')) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        // Filtrar solo productos de Productos y Servicios
        const productosYServiciosProducts = allProducts.filter(product => 
          product.category.toLowerCase().includes('productos y servicios')
        );
        
        setProducts(productosYServiciosProducts);
        setError(null);
      } catch (err) {
